...
class ResultProcessor:
    ...
    def parse_llm_results(self, llm_output: str, questions: List[str]) -> Dict[str, str]:
            ...
            for section in sections:
                ...
                if any(keyword in section_lower for keyword in question_keywords):
                    ...
                    for line in lines:
                        ...
                        if len(line) > len(question) + 5 and not line.lower().startswith(question.lower()[:10]):
                            # Clean up the answer
                            line = re.sub(r'^[\d\.\-\*\s]*', '', line)  # Remove leading numbers/bullets
                            line = re.sub(r'^(answer|a):\s*', '', line, flags=re.IGNORECASE)  # Remove "Answer:"
                            if line:
                                answer = line
                                break
                    break
    ...
    def combine_results(self, papers_data: List[Dict], llm_results: List[str], questions: List[str]) -> List[Dict]:
    ...
    def generate_markdown_table(self, results: List[Dict], questions: List[str], include_header: bool = True) -> str:
        ...
        if include_header:
            # Prepare headers
            headers = ["Paper Title", "arXiv Link"] + [q.replace("\n", " ") for q in questions]
    ...
    def questions_match(self, existing_questions: List[str], config_questions: List[str]) -> bool:
    ...
    def save_results(self, results: List[Dict], questions: List[str], base_filename: str) -> str:
        ...
        if should_append:
            ...
            markdown_table = self.generate_markdown_table(results, questions, include_header=False)
            with open(output_path, "a", encoding="utf-8") as f:
                f.write(markdown_table)
            logging.info(f"Appended {len(results)} new results to existing file: {output_path}")
        else:
            # New file mode: include full table with header
            markdown_table = self.generate_markdown_table(results, questions, include_header=True)
...
if __name__ == "__main__":
    ...
    # Mock test data
    papers_data = [
        {"title": "Test Paper", "arxiv_url": "http://arxiv.org/abs/test", "abstract": "Test abstract"}
    ]
    ...
    questions = ["What is the main physics phenomenon", "Is this related to dark matter"]
