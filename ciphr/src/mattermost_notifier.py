...
class MattermostNotifier:
    ...
    def post_ml4dm_findings(self, ml4dm_papers: List[Dict], repo_link: str) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                logging.info(f"Successfully posted ML4DM findings to Mattermost. Found {len(ml4dm_papers)} papers.")
                return True
            else:
                logging.error(f"Failed to post to Mattermost. Status: {response.status_code}, Response: {response.text}")
                return False
    ...
    def _extract_ml_techniques(self, ml_answer: str) -> str:
        ...
        if ml_answer.lower().startswith(('yes', 'this paper')):
            ...
            for delimiter in ['. The main ML techniques', ': ', '. Techniques', '. Methods']:
...
def check_ml4dm_papers(papers_data: List[Dict], llm_results: List[str]) -> List[Dict]:
    ...
    for i, (paper_data, llm_result) in enumerate(zip(papers_data, llm_results)):
        try:
            ...
            if isinstance(llm_result, str):
                ...
                if json_match:
                    ...
                    logging.warning(f"Could not extract JSON from LLM result for paper {i+1}")
                    continue
            else:
                llm_json = llm_result
            ...
            if _indicates_ml_usage(ml4dm_answer):
                ...
                logging.info(f"Paper {i+1} flagged as ML4DM: {paper_data['title'][:50]}...")
...
if __name__ == "__main__":
    ...
    message = notifier._format_ml4dm_message(test_papers, "https://github.com/test/repo")
    print("Test message:")
    print(message)