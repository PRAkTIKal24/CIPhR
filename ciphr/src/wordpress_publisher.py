...
class WordPressPublisher:
    ...
    def convert_markdown_to_html(self, markdown_content: str) -> str:
        ...
        if not markdown_content.strip():
            return f"<p>No research data available.</p>"
        ...
        try:
            ...
            html_body = html_body.replace('<table>', '<table class="wp-block-table is-style-stripes">')
            ...
            logging.warning(f"Pandoc conversion failed: {e}, falling back to custom parser")
    ...
    def _convert_markdown_to_html_fallback(self, markdown_content: str) -> str:
        ...
        if not header_line:
            return f"<p>No valid table structure found.</p>"
        
        # Parse header
        headers = self._parse_table_row(header_line)
        if not headers:
            return f"<p>No valid table headers found.</p>"
        ...
        for line in data_lines:
            ...
            if cells:
                ...
                for i, cell in enumerate(cells[:len(headers)]):  # Don't exceed header count
    ...
    def _parse_table_row(self, line: str) -> list:
        ...
        return [cell for cell in cells if cell or len(cells) <= 1]  # Keep empty cells only if single cell
    ...
    def find_ciphr_post(self) -> Optional[int]:
        ...
        try:
            ...
            params = {
                'search': 'CIPhR Research Insights',
                'per_page': 10
            }
    ...
    def create_or_update_post(self, markdown_content: str, paper_count: int = 0) -> bool:
        """
        ...
        try:
            ...
            if existing_post_id:
                ...
                response = requests.post(update_url, headers=self.headers, json=post_data, timeout=60)
                ...
                if response.status_code == 200:
                    ...
                    logging.error(f"Failed to update WordPress post: {response.status_code} - {response.text}")
                    return False
            else:
                # Create new post
                create_url = f"{self.api_base}/posts"
                response = requests.post(create_url, headers=self.headers, json=post_data, timeout=60)
                ...
                if response.status_code == 201:
                    ...
                    logging.error(f"Failed to create WordPress post: {response.status_code} - {response.text}")
                    return False
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            response = requests.get(test_url, headers=self.headers, params=params, timeout=30)
            
            if response.status_code == 200:
                logging.info("WordPress API connection successful")
                return True
            else:
                logging.error(f"WordPress API connection failed: {response.status_code}")
                return False
...
def publish_research_insights(
    ...
    try:
        ...
        if success:
            logging.info(f"Successfully published research insights from {actual_file_path} to WordPress")
        else:
...
def find_latest_output_file(base_file_path: str) -> str:
    ...
    if matching_files:
        ...
        logging.info(f"LLM questions appear to have changed. Using latest output file: {latest_file}")
        return latest_file
