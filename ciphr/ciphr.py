...
def main():
    ...
    should_append = actual_filename == args.output_filename and os.path.exists(
        output_path
    )
    ...
    if should_append:
        ...
        logging.info(
            f"Analysis complete. {len(all_results)} new results appended to existing file: {output_path}"
        )
    else:
        ...
        if actual_filename != args.output_filename:
            logging.info(
                f"Analysis complete. New file created with date suffix: {output_path}"
            )
        else:
